      ...
      <div class="p-4 sm:p-6 lg:p-8 overflow-x-auto">
        <div class="max-h-[400px] overflow-y-auto rounded-xl border border-gray-200">
          <table class="min-w-[1000px] w-full border-collapse text-center">
            ...
            <tbody>
              <tr v-for="(user, index) in displayedRankingData" :key="user.memberID" :class="[
                ...
                <td class="border border-gray-200 p-3 sm:p-4 relative">
                  <img :src="user.avtURL" alt="avatar" :class="[
                  ...
                  <div v-if="user.memberID === currentUser?.memberID"
                ...
                <td class="border border-gray-200 p-3 sm:p-4">
                  <div class="flex flex-col items-center">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
                      <div class="h-2 rounded-full bg-gradient-to-r from-red-600 to-red-400 transition-all duration-300"
                        :style="{ width: calculateLevelProgress(user) + '%' }"></div>
                    </div>
                  </div>
                </td>
                <td class="border border-gray-200 p-3 sm:p-4 font-bold text-base sm:text-lg"
                  :class="user.memberID === currentUser?.memberID ? 'text-red-600' : 'text-gray-900'">
                  {{ user.points }}
                  <span :class="user.memberID === currentUser?.memberID ? 'text-red-500' : 'text-gray-700'"
                    class="text-sm">
                    pts
                  </span>
                </td>
